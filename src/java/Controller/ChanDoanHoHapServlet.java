 ...
 */
@WebServlet(name = "ChanDoanHoHapServlet", urlPatterns = {"/ChanDoanHoHapServlet"})
public class ChanDoanHoHapServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ChanDoanHoHapServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ChanDoanHoHapServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            benh = ChanDoanBenh.chanDoan(hh);
        } catch (Exception ex) {
            Logger.getLogger(ChanDoanServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
        if (user != null){
            String code =Database.UpdateHistory(user.getName(), benh, user.getSessionCode(),"Hô hấp");
